...
class DirichletCategoricalSampler(torch.nn.Module):
	def __init__(self,
			input_size,
			mlp_hidden_size,
			num_categories,
		...
		super(DirichletCategoricalSampler, self).__init__()
		self.num_categories = num_categories
		self.to_logits = MLP(input_size, mlp_hidden_size, num_categories)
		self.min_temperature = min_temperature
		self.epoch_init_iter_counts = epoch_init_iter_counts
		self.iter_counts = epoch_init_iter_counts
	...
	def forward(self, parameter_seed):
		...
		"""
		logits = self.to_logits(parameter_seed)
		return logits
	...
	def kl_divergence(self, logits, entire_data_size):
		...
		posterior_concentration = posterior_shape*entire_data_size + self.prior_concentration
		...
		Eq_log_q_z = (q_z * q_z.log()).sum()

		# Eq[log p(z|pi)]
		Eq_log_p_z = (q_z * expected_log_pi[None,:]).sum()

		batch_size = logits.size(0)

		kl = (Eq_log_q_pi - Eq_log_p_pi) * (batch_size / entire_data_size) + Eq_log_q_z - Eq_log_p_z
		return kl
	...
	def increment_iter_counts(self):
		self.iter_counts += 1
		if self.iter_counts % self.temperature_update_freq == 0:
			self.update_temperature()

	def update_epoch_init_iter_counts(self):
		self.epoch_init_iter_counts = self.iter_counts
