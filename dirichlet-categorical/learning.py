...
class Learner(object):
	...
	def train(self, dataloader):
		...
		for batch_ix,(packed_input, is_offset, speaker, _) in enumerate(dataloader, 1):
			...
			logger.info('{batch_ix}/{num_batches} training batches complete. mean loss: {loss:5.4f}. Perplexity of the posterior clustering probs.: {cluster_perplex:5.4f}. Perplexity of the posterior Dirichlet shape: {shape_perplex:5.4f}'.format(
							batch_ix=batch_ix,
	...
	def retrieve_model(self, checkpoint_path = None, device='cpu'):
		...
		self.feature_sampler = model.DirichletCategoricalSampler(**checkpoint['feature_sampler_init_parameters'])
