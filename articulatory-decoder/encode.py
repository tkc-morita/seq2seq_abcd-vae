...
class Encoder(learning.Learner):
	...
	def encode(self, data, is_packed = False, to_numpy = True):
		...
		if to_numpy:
			params = {name:(p.data.cpu().numpy() for p in ps) for name,ps in params.items()}
		return params
	...
	def encode_dataset(self, dataset, to_numpy = True, parameter_ix2name=None, batch_size=1):
		...
		for data, _, _, _, _, ix_in_list in dataloader:
			...
			for name,ps in params.items():
				for parameter_ix,p in enumerate(ps):
					if parameter_ix in parameter_ix2name:
						parameter_name = parameter_ix2name[parameter_ix]
					else:
						parameter_name = parameter_ix
					parameter_name = '{name}_{parameter_name}'.format(name=name, parameter_name=parameter_name)
...
def get_parameters():
	...
	par_parser.add_argument('--context_length', type=float, default=0, help='Length of the prefix and suffix sound wave in sec.')
