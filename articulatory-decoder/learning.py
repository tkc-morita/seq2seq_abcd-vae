...
class Learner(object):
	def __init__(self,
			...
			f0_lower_ix=0,
			f0_upper_ix=None,
			source_decay_per_actave=(-6 * np.log(10, dtype=np.float32) / 20) / 15 * np.log(2, dtype=np.float32),
			silence=- 15 * np.log(2, dtype=np.float32),
		...
		if self.retrieval:
			...
			self.decoder = model.RNN_Variational_Decoder(
							rnn_input_size,
							decoder_rnn_hidden_size,
							mlp_hidden_size,
							feature_size+context_feature_size*2,
							emission_distr_name=emission_distribution,
							rnn_type=decoder_rnn_type,
							input_dropout=decoder_input_dropout,
							self_feedback=decoder_self_feedback,
							esn_leak=esn_leak,
							bidirectional=bidirectional_decoder,
							right2left_weight=right2left_decoder_weight,
			...
			self.feature_sampler = model.MultipleSamplers(self.encoder.hidden_size_total, mlp_hidden_size, feature_size, sampler_names=self.decoder.feature2hidden.keys(), distribution_name=feature_distribution)
			...
			logger.info('Decoder is based on the souce-filter model.')
			if filter_conv_kernel_size>1:
				logger.info('The peaks of the filters are convolved with the 1-valued window of length {}'.format(filter_conv_kernel_size))
	...
	def train(self, dataloader):
		...
		for batch_ix,(packed_input, is_offset, packed_prefix, packed_suffix, speaker, _) in enumerate(dataloader, 1):
			...
			kl_loss_per_batch += sum(self.feature_sampler.kl_divergence(feature_params).values())
		...
		with torch.no_grad():
			for batch_ix, (packed_input, is_offset, packed_prefix, packed_suffix, speaker, _) in enumerate(dataloader, 1):
				...
				if not self.prefix_encoder is None:
					...
					suffix_features = self.suffix_encoder(packed_suffix)
					features = torch.cat([features, prefix_features, suffix_features], dim=-1) # This concat order is arbitrary.
				kl_loss += sum(self.feature_sampler.kl_divergence(feature_params).values()).item()
				emission_loss_per_batch, end_prediction_loss_per_batch, _, _, _ = self.decoder(features, batch_sizes=packed_input.batch_sizes, speaker=speaker, ground_truth_out=packed_input.data, ground_truth_offset=is_offset.data)
				emission_loss += emission_loss_per_batch.item()
				end_prediction_loss += end_prediction_loss_per_batch.item()
	...
	def retrieve_model(self, checkpoint_path = None, device='cpu'):
		...
		self.feature_sampler = model.MultipleSamplers(**checkpoint['feature_sampler_init_args'])
...
def get_parameters():
	...
	par_parser.add_argument('--f0_lower_bound', type=float, default=250, help="Lower bound on decoder's F0 (Hz).")
	par_parser.add_argument('--f0_upper_bound', type=float, default=550, help="Upper bound on decoder's F0 (Hz).")
	par_parser.add_argument('--source_decay', type=float, default=6, help='Per-octave decay of the sound source (+radiation). In dB.')
	par_parser.add_argument('--filter_conv_kernel_size', type=int, default=1, help="Lenght of the convolution window for the decoder's filter.")
...
if __name__ == '__main__':
	...
	# From dB to ln.
	source_decay_per_actave = (parameters.source_decay * np.log(10, dtype=np.float32) / 20)
	# Normalize
	source_decay_per_actave /= parameters.data_normalizer
	silence = np.log(parameters.epsilon, dtype=np.float32)
	...
	learner = Learner(
				...
				source_decay_per_actave=source_decay_per_actave,
				silence=silence,
				filter_conv_kernel_size=parameters.filter_conv_kernel_size
				)
	...
	logger.info("Decoder's F0 is bounded in the range [{lower}Hz,{upper}Hz], or [{lower_ix},{upper_ix}) in terms of indices.".format(lower=f0_lower_ix*freq_step, upper=(f0_upper_ix-1)*freq_step, lower_ix=f0_lower_ix, upper_ix=f0_upper_ix))
	logger.info("Decoder's voice source decays by {decay}dB/octave ({transformed}/octave in the log space) in the frequency axis.".format(decay=parameters.source_decay, transformed=source_decay_per_actave))
	logger.info("log({epsilon}) is taken as the silence.".format(epsilon=parameters.epsilon))
