...
class Encoder(learning.Learner):
	...
	def encode(self, data, is_packed = False, to_numpy = True):
		...
		with torch.no_grad():
			...
			features = self.feature_sampler.to_code_like(last_hidden)
		if to_numpy:
			features = (p.data.cpu().numpy() for p in features)
		return features
	...
	def encode_dataset(self, dataset, save_path, to_numpy = True, batch_size=1):
		...
		for data, _, _, ix_in_list in dataloader:
			features = self.encode(data, is_packed=True, to_numpy=to_numpy)
			df_encoded = pd.DataFrame(features)
			df_encoded.loc[:,'data_ix'] = ix_in_list
			df_encoded = df_encoded.melt(id_vars=['data_ix'], var_name='dimension', value_name='feature_value')
