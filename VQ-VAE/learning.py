...
class Learner(object):
	def __init__(self,
		...
		if self.retrieval:
			...
			self.feature_sampler = model.VQ_VAE(self.encoder.hidden_size_total, mlp_hidden_size, num_feature_categories, feature_dim)
	...
	def train(self, dataloader):
		...
		for batch_ix,(packed_input, is_offset, speaker, _) in enumerate(dataloader, 1):
			...
			prequantized = self.feature_sampler(last_hidden)
			features,regularize_term_per_batch = self.feature_sampler.sample(prequantized)
			...
			loss = emission_loss_per_batch + end_prediction_loss_per_batch + regularize_term_per_batch
			...
			regularize_term += regularize_term_per_batch.item()

			logger.info('{batch_ix}/{num_batches} training batches complete. mean loss: {loss:5.4f}.'.format(
							batch_ix=batch_ix,
		...
		regularize_term /= num_strings
		mean_loss = emission_loss + end_prediction_loss + regularize_term
		...
		logger.info('mean training KL (per string): {:5.4f}'.format(regularize_term))
	...
	def test_or_validate(self, dataloader):
		...
		with torch.no_grad():
			for batch_ix, (packed_input, is_offset, speaker, _) in enumerate(dataloader, 1):
				...
				prequantized = self.feature_sampler(last_hidden)
				features,regularize_term_per_batch = self.feature_sampler.sample(prequantized)
				regularize_term += regularize_term_per_batch
				emission_loss_per_batch, end_prediction_loss_per_batch, _, _, _ = self.decoder(features, batch_sizes=packed_input.batch_sizes, speaker=speaker, ground_truth_out=packed_input.data, ground_truth_offset=is_offset.data)
				emission_loss += emission_loss_per_batch.item()
				end_prediction_loss += end_prediction_loss_per_batch.item()
		...
		logger.info('mean validation KL (per string): {:5.4f}'.format(regularize_term))
	...
	def retrieve_model(self, checkpoint_path = None, device='cpu'):
		...
		self.feature_sampler = model.VQ_VAE(**self.checkpoint['feature_sampler_init_parameters'])
