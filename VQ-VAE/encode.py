...
class Encoder(learning.Learner):
	...
	def encode(self, data, is_packed = False, to_numpy = True):
		...
		with torch.no_grad():
			...
			categories = self.feature_sampler.categorize(prequantized)
		if to_numpy:
			categories = categories.data.cpu().numpy()
		return categories
	...
	def encode_dataset(self, dataset, save_path, to_numpy = True, batch_size=1):
		...
		for data, _, _, ix_in_list in dataloader:
			categories = self.encode(data, is_packed=True, to_numpy=to_numpy)
			df_encoded = pd.DataFrame(categories, columns=['category_ix'])
